...
void ui_display_source_selection(WINDOW *win, AppState *app)
{
    ...
    if (app->source_sort == 't') sort_label = "Title";
    else if (app->source_sort == 'a') sort_label = "Artist";
    char sort_text[STR_MEDIUM];
    snprintf(sort_text, sizeof(sort_text), "Select Source:  [sorted by %s]", sort_label);
    ...
    for (int i = start; i < end; i++) {
        ...
        mvwaddnstr(win, 8 + display_i, 4, line, width - 6);
...
void ui_display_player_selection(WINDOW *win, AppState *app)
{
    ...
    for (int i = 0; i < app->players_count; i++) {
        ...
        snprintf(line, sizeof(line), "%s%s (%s)", marker,
                 app->players[i]->name, app->players[i]->host_name);
        if (i == app->selected_index) wattron(win, COLOR_PAIR(2));
        mvwaddstr(win, 4 + i, 4, line);
        if (i == app->selected_index) wattroff(win, COLOR_PAIR(2));
    }
    int height, width;
    getmaxyx(win, height, width);
    (void)width;
