...
static size_t meta_write_cb(void *ptr, size_t size, size_t nmemb,
                            void *userdata) {
...
static bool meta_http_get(const char *url, long timeout, const char *ua,
                          HttpBuffer *out) {
...
static char *openai_build_request(const char *model, const char *prompt,
  ...
  *out_headers = curl_slist_append(*out_headers, auth);
  *out_headers =
      curl_slist_append(*out_headers, "Content-Type: application/json");
  return json_str;
}

static bool openai_extract_response(const char *data, char *response,
                                    size_t resp_size) {
...
static bool openai_post(const char *api_key, const char *model,
  ...
  struct curl_slist *headers = NULL;
  char *json_str = openai_build_request(model, prompt, max_tokens, temperature,
                                        api_key, &headers);
  ...
  bool ok = openai_extract_response(buf.data, response, resp_size);
  free(buf.data);
  return ok;
}
...
static bool mb_parse_cached_info(char *cached, CombinedInfo *out) {
...
static void mb_extract_metadata(cJSON *best, CombinedInfo *out) {
  ...
  if (!label_info || !cJSON_IsArray(label_info) ||
      cJSON_GetArraySize(label_info) == 0)
    return;
...
static void mb_extract_top_genres(cJSON *tags, char *genre_buf,
                                  size_t buf_size) {
  ...
  for (int t = 0; t < int_min(TOP_GENRE_COUNT, tag_n); t++) {
    ...
    if (tname && cJSON_IsString(tname)) {
      if (taken > 0)
        strncat(genre_buf, ", ", buf_size - strlen(genre_buf) - 1);
      strncat(genre_buf, tname->valuestring, buf_size - strlen(genre_buf) - 1);
      taken++;
    }
  }
}

static void mb_fetch_genre_tags(cJSON *best, CombinedInfo *out) {
  ...
  snprintf(tag_url, sizeof(tag_url), "%s/release-group/%s?inc=tags&fmt=json",
           MUSICBRAINZ_BASE_URL, rg_id->valuestring);
  HttpBuffer tbuf = {0};
  if (!meta_http_get(tag_url, HTTP_TIMEOUT_MUSICBRAINZ, MUSICBRAINZ_USER_AGENT,
                     &tbuf))
    return;
  ...
  if (tags && cJSON_IsArray(tags) && cJSON_GetArraySize(tags) > 0) {
    ...
    mb_extract_top_genres(tags, genre_buf, sizeof(genre_buf));
    if (genre_buf[0])
...
static cJSON *mb_find_best_release(cJSON *releases, const char *artist,
                                   const char *album) {
  cJSON *best = NULL;
  int best_score = -1;
...
static bool mb_process_search_results(char *data, const char *artist,
                                      const char *album, const char *cache_key,
                                      CombinedInfo *out) {
  cJSON *root = cJSON_Parse(data);
  if (!root)
    return false;
  ...
  cJSON *best = mb_find_best_release(releases, artist, album);
  if (!best) {
    cJSON_Delete(root);
    return false;
  }
  mb_extract_metadata(best, out);
  mb_fetch_genre_tags(best, out);
  char cache_val[STR_LONG];
  snprintf(cache_val, sizeof(cache_val), "%s|%s|%s", out->year, out->label,
           out->genre);
...
static bool mb_search_query(const char *query_fmt, const char *artist,
                            const char *album, const char *cache_key,
                            CombinedInfo *out) {
  rate_limit();
  char query[STR_URL];
  snprintf(query, sizeof(query), query_fmt, artist, album);
  ...
  HttpBuffer buf = {0};
  if (!meta_http_get(url, HTTP_TIMEOUT_MUSICBRAINZ, MUSICBRAINZ_USER_AGENT,
                     &buf))
    return false;

  bool ok = mb_process_search_results(buf.data, artist, album, cache_key, out);
  free(buf.data);
  return ok;
}

static bool mb_get_album_info(const char *artist, const char *album,
                              CombinedInfo *out) {
  if (!artist[0] || !album[0])
    return false;
  ...
  if (cached) {
    mb_parse_cached_info(cached, out);
    free(cached);
    return true;
  }
  if (cache_has(meta_cache, cache_key))
    return false;

  const char *queries[] = {"artist:\"%s\" AND release:%s",
                           "artist:%s release:%s"};

  for (int q = 0; q < 2; q++) {
    if (mb_search_query(queries[q], artist, album, cache_key, out))
      return true;
  }
...
void metadata_init(void) {
  meta_logger =
      logger_create(LOG_FILE_METADATA, LOG_MAX_BYTES, LOG_BACKUP_COUNT);
...
static void parse_combined_json(cJSON *j, CombinedInfo *out) {
  ...
  if (y && cJSON_IsString(y))
    safe_strcpy(out->year, y->valuestring, sizeof(out->year));
  if (l && cJSON_IsString(l))
    safe_strcpy(out->label, l->valuestring, sizeof(out->label));
  if (g && cJSON_IsString(g))
    safe_strcpy(out->genre, g->valuestring, sizeof(out->genre));
...
static bool parse_cached_combined(char *cached, CombinedInfo *out) {
  cJSON *j = cJSON_Parse(cached);
  if (j) {
    parse_combined_json(j, out);
    cJSON_Delete(j);
  }
  free(cached);
  return true;
...
static bool parse_combined_response(char *response, const char *cache_key,
                                    CombinedInfo *out) {
...
static bool fetch_combined_from_openai(const char *title, const char *artist,
                                       const char *album, const char *api_key,
                                       const char *system_prompt,
                                       const char *model, const char *cache_key,
                                       CombinedInfo *out) {
  char track_instr[STR_MEDIUM] =
      "a short paragraph (2-4 sentences) about this specific song";
  if (system_prompt && system_prompt[0]) {
    strncat(track_instr, ". ", sizeof(track_instr) - strlen(track_instr) - 1);
    strncat(track_instr, system_prompt,
            sizeof(track_instr) - strlen(track_instr) - 1);
  }
  char prompt[STR_TEXT];
  snprintf(prompt, sizeof(prompt), COMBINED_INFO_PROMPT_FMT, title, artist,
           album, track_instr);
  ...
  if (!openai_post(api_key, use_model, prompt, COMBINED_INFO_MAX_TOKENS,
    ...
    return false;
  if (!parse_combined_response(response, cache_key, out))
    return false;
  return true;
}
...
bool metadata_get_combined(const char *title, const char *artist,
  ...
  char cache_key[STR_LONG];
  snprintf(cache_key, sizeof(cache_key), "combined|%s|%s|%s", title, artist,
           album);
  char *cached = cache_get(meta_cache, cache_key);
  if (cached)
    return parse_cached_combined(cached, out);
  if (cache_has(meta_cache, cache_key))
    return false;
  ...
  if (!fetch_combined_from_openai(title, artist, album, api_key, system_prompt,
                                  model, cache_key, out)) {
    LOG_ERROR(meta_logger, "Combined AI request/parse failed, falling back");
    mb_get_album_info(artist, album, out);
  }
  return true;
}
...
char *metadata_get_station_info(const char *station_name, const char *api_key,
                                const char *model) {
  if (!station_name || !station_name[0] || !api_key || !api_key[0])
    return NULL;
...
char *metadata_get_lyrics(const char *title, const char *artist) {
  if (!title || !title[0] || !artist || !artist[0])
    return NULL;
  ...
  char url[STR_URL * 2];
  snprintf(url, sizeof(url), "%s?artist_name=%s&track_name=%s", LRCLIB_API_URL,
           enc_artist, enc_title);

  HttpBuffer buf = {0};
  if (!meta_http_get(url, HTTP_TIMEOUT_LYRICS, MUSICBRAINZ_USER_AGENT, &buf))
    return NULL;
